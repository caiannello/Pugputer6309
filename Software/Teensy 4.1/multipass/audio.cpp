...
//
//  Services for sound input, output, and synthesis.
//
//  I'm thinking about letting the host CPU have some arbitrary-waveform 
//  oscillator channels, like a PC Engine / Turbografx 16 game console, and
//  some waveform instruments, like an Amiga MOD Player, but for now, we just 
//  have the guitar synth demo from the Teensy audio library. (Will let
//  the host CPU be able to play with that.)
//
// ----------------------------------------------------------------------------
#include "audio.h"
// ----------------------------------------------------------------------------
...
void aud_init() 
{
  AudioMemory(15);
  sgtl5000_1.enable();
  sgtl5000_1.volume(0.4); // was 0.6, initially, which is quite loud on my headphones
  mixer1.gain(0, 0.15);
  mixer1.gain(1, 0.15);
  mixer1.gain(2, 0.15);
  mixer1.gain(3, 0.15);
...
// ----------------------------------------------------------------------------
// called by mainloop to handle audio housekeeping stuff
// ----------------------------------------------------------------------------
void aud_service(void) 
{
  ...
  if (chordnum == 0) {
    chord = Cmajor;
    Serial.print("C major ");
    chordnum = 1;
  } else if (chordnum == 1) {
    chord = Gmajor;
    Serial.print("G major ");
    chordnum = 2;
  } else if (chordnum == 2) {
    chord = Aminor;
    Serial.print("A minor ");
    chordnum = 3;
  } else {
    chord = Eminor;
