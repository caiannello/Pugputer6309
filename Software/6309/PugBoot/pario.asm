...
;
; The housekeeping function, pa_svc, gets called by mainloop, and it will
...
sc_loop     lda  ,X+        ; Send all bytes out right in this loop. 
            sta  PA_DAT     ; (if this causes unacceptable pauses,
                            ; we can change it to be a reentrant svc
                            ; called by the mainloop.)

            ;jsr  con_puthbyte

            LDW  #500       ; delay loop per tx byte (hopefully temporary)
delloop     subw #1
            bne delloop            
