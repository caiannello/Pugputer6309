...
;
; -----------------------------------------------------------------------------
; Current Memory Map for the Pugputer. (The address decoding for IO space 
; needs improvement: 512 bytes per peripheral is too much.)
;
; start     end     use
; 0x0000    0x7fff  RAM
; 0x8000    0xBFFF  RAM 
; 0XB000    0XEFFF  ROM, EXTENDED BASIC, ETC.
; 0xF000    0xF1FF  ONBOARD UART
; 0xF200    0xF3FF  CARD, AUDIO, SDCARD, IO
; 0xF400    0xF5FF  CARD, VIDEO
; 0XF600    0XF7FF  CARD, RESERVED
; 0XF800    0XFFFF  ROM, MONITOR, BIOS, AND INTERRUPT VECTORS
; 
; -----------------------------------------------------------------------------
...
VD9       RMB  1               
VDA       RMB  1               
SW3VEC    RMB  3              JMP $009B   <-- LOADED TO THIS RAM ADRS ON COLD START
SW2VEC    RMB  3              JMP $009E 
SWIVEC    RMB  3              JMP $00A1 
NMIVEC    RMB  3              JMP $00A4 
IRQVEC    RMB  3              JMP $00A7    
FRQVEC    RMB  3              JMP $00AA 
...
DUMP_ADRS RMB  2              ADDRESS USED BY HEXDUMP ROUTINE
    INCLUDE vdp_globals.asm         ; V9958 VIDEO DISPLAY PROCESSOR GLOBALS
    INCLUDE megaudio_globals.asm    ; audio card init
...
V_RESET   EQU  $FFFE           
          ...
          ORG  $B000         ; START OF ROM CONTENTS -----------------  

    ; JUMP TABLE FOR SOME USEFUL FUNCTIONS
    
JT0       JMP VDP_ENABLE_TEXT2_DISPLAY_WITH_VBLANK
JT1       JMP VDP_ENABLE_TEXT2_DISPLAY_WITHOUT_VBLANK

    INCLUDE vdp.asm         ; V9958 VIDEO DISPLAY PROCESSOR
    INCLUDE uart.asm        ; R65C51P2 SERIAL UART
    INCLUDE megaudio.asm

RESVEC:
LA00E     LDS  #LINBUF+LBUFMX+1 SET STACK TO TOP OF LINE INPUT BUFFER
          LDMD #$01           ; switch to 6309 native mode
          JSR  UART_INIT      ;initialize SERIAL PORT
...
LA077     CLR  ,--X           MOVE POINTER DOWN TWO-CLEAR BYTE 
          ...
          ; TODO: THIS IS HACKY:
          LDA  #0             IN CASE THE LED BOARD IS CONNECTED,
          STA  $F200          TURN OFF A BUNCH OF LEDS INITIALLY
...
L8033     JSR  KEYIN
          BEQ  L8033
          JSR  VDP_INIT       ; initialize V9958 display processor
          JSR  VDP_SHOW_STARTUP_MESSAGE   
          ;JSR  AUD_INIT       ; INITIALIZE AUDIO CARD
...
BAWMST    NOP                 NOP REQ’D FOR WARM START  
          ...
          JSR  VDP_INIT       ; initialize V9958 display processor
          LDA  VDP_VBLANK_ENABLED
          BEQ  SKIPVBLANK
          JSR VDP_ENABLE_TEXT2_DISPLAY_WITH_VBLANK
SKIPVBLANK:
          ;JSR  AUD_INIT       ; INITIALIZE AUDIO CARD
...
; IO SPACE IS AT F000 - F7FF, SO WE CAN PUT COMMENTS IN ROM HERE
        ORG  $F000
        FCN "Microsoft Extended BASIC for the"
        FCN "TRS-80 Color Computer, 1982.    "
        FCN "Stripped of CoCo-specific stuff "
        FCN "by Grant Searle, with further   "
        FCN "adaptations by Tom LeMense      "
        FCN "(tomcircuit on github)          "
        FCN "Graphics and sound stuff added  "
        FCN "to work with Pugputer 6309 by   "
        FCN "Craig Iannello 2022-2023.       "
        FCN "(caiannello on Github and gmail,"
        FCN "AppliedCryogenics on YouTube.)  "
        FCN "Toolchain: lwtools ver 4.10     "
        ORG  $F800
...
LBFF6     FDB  VDP_FIRQ       ; FIRQ
;LBFF8     FDB  IRQVEC         ; IRQ 
LBFF8     FDB  AUD_IRQ        ; IRQ 
